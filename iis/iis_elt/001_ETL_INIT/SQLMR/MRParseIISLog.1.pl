...
# 11/21/2010 - We dont emit "Discard" rows anymore - speeds up quite a bit as we ignore that in the workflow anyways
# 11/22/2010 - Add ugrp field in query string
#
...
while (my $line=<STDIN>) {
	...
	while ($CandidateField = shift (@Candidates)){
	    if (index($CandidateField, '+') > -1){$BestCandidate = ConcatFields($CandidateField)}
		  ...
		  if ($Value{$KeyFields[0],$KeyFields[1]}){$BestCandidate = $Value{$KeyFields[0],$KeyFields[1]};
								       last;
								   };
	      };
	};
	...
	if ($TruncOutPattern[$index]){
	    ...
	    $BestCandidate = shift(@TruncatedOutField) || "";
	};
