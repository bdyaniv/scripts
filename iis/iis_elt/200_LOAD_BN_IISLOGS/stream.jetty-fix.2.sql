...
SELECT 
	...
	case when userid ~ '[a-z|A-Z]' then userid 
             when c_ip = '-' then (-1*extract(epoch from date_time::timestamp))::varchar
             else (-1*replace(c_ip,'.','')::bigint)::varchar 
        end,
	c_ip,
	sc_status,
...
FROM STREAM(
     ON (SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
                || line 
         FROM 
           iis_logs_raw_child_%CUR_YYYYMMDD%
         WHERE 
           hit_timestamp >= ('%CUR_YYYYMMDD% 0:0:0'::timestamp + interval '5 hours')
         UNION ALL
         SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
                || line
         FROM
           iis_logs_raw_child_%NEXT_YYYYMMDD%
         WHERE
           hit_timestamp < ('%NEXT_YYYYMMDD% 00:00:00'::timestamp + interval '5 hours')
        )
--    ON (SELECT * from public.iis_logs_raw)
    SCRIPT('perl MRParseIISLog.4.pl') -- Change it back to .1.pl if it doesnt work
    OUTPUTS(
	'outfile            varchar', 
	'userid             varchar', 
	'c_ip               varchar', 
...
ALTER TABLE iis_child.iis_logs_raw_child_%PPREV_YYYYMMDD% COMPRESS LOW;
