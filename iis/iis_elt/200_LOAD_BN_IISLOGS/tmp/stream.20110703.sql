...
FROM STREAM(
     ON (SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
         ...
         WHERE 
           hit_timestamp >= ('20110703 0:0:0'::timestamp + interval '5 hours')
         UNION ALL
         ...
         WHERE
           hit_timestamp < ('20110704 00:00:00'::timestamp + interval '5 hours')
        )
...
ALTER TABLE iis_child.iis_logs_raw_child_20110701 COMPRESS HIGH;
