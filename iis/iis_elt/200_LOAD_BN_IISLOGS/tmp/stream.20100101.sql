...
FROM STREAM(
     ON (SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
                || line 
         FROM 
           iis_logs_raw_child_20100101
         WHERE 
           hit_timestamp >= ('20100101 0:0:0'::timestamp + interval '5 hours')
         UNION
         SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
                || line
         FROM
           iis_logs_raw_child_20100102
         WHERE
           hit_timestamp < ('20100102 00:00:00'::timestamp + interval '5 hours')
        )
...
ALTER TABLE iis_child.iis_logs_raw_child_20091230 COMPRESS HIGH;
