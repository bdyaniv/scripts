...
FROM STREAM(
     ON (SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
                || line 
         FROM 
           iis_logs_raw_child_20101217
         WHERE 
           hit_timestamp >= ('20101217 0:0:0'::timestamp + interval '5 hours')
         UNION ALL
         SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
                || line
         FROM
           iis_logs_raw_child_20101218
         WHERE
           hit_timestamp < ('20101218 00:00:00'::timestamp + interval '5 hours')
        )
...
ALTER TABLE iis_child.iis_logs_raw_child_20101215 COMPRESS HIGH;
