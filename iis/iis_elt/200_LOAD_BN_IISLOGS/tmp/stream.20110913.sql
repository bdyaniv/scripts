...
FROM STREAM(
     ON (SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
                || line 
         FROM 
           iis_logs_raw_child_20110913
         WHERE 
           hit_timestamp >= ('20110913 0:0:0'::timestamp + interval '5 hours')
         UNION ALL
         ...
         WHERE
           hit_timestamp < ('20110914 00:00:00'::timestamp + interval '5 hours')
        )
...
ALTER TABLE iis_child.iis_logs_raw_child_20110911 COMPRESS HIGH;
