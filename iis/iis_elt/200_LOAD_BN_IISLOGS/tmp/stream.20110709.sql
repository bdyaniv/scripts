...
FROM STREAM(
     ON (SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
         ...
         WHERE 
           hit_timestamp >= ('20110709 0:0:0'::timestamp + interval '5 hours')
         UNION ALL
         SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
                || line
         FROM
           iis_logs_raw_child_20110710
         WHERE
           hit_timestamp < ('20110710 00:00:00'::timestamp + interval '5 hours')
        )
...
ALTER TABLE iis_child.iis_logs_raw_child_20110707 COMPRESS HIGH;
