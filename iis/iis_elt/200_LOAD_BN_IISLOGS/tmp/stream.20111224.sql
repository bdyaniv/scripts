...
FROM STREAM(
     ON (SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
                || line 
         FROM 
           iis_logs_raw_child_20111224
         WHERE 
           hit_timestamp >= ('20111224 0:0:0'::timestamp + interval '5 hours')
         UNION ALL
         SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
                || line
         FROM
           iis_logs_raw_child_20111225
         WHERE
           hit_timestamp < ('20111225 00:00:00'::timestamp + interval '5 hours')
        )
...
ALTER TABLE iis_child.iis_logs_raw_child_20111222 COMPRESS LOW;
