...
FROM STREAM(
     ON (SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
         ...
         WHERE 
           hit_timestamp >= ('20111018 0:0:0'::timestamp + interval '5 hours')
         UNION ALL
         ...
         WHERE
           hit_timestamp < ('20111019 00:00:00'::timestamp + interval '5 hours')
        )
...
ALTER TABLE iis_child.iis_logs_raw_child_20111016 COMPRESS HIGH;
