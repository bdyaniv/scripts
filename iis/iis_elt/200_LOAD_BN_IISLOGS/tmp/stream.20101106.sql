...
SELECT 
	...
	ugrp
FROM STREAM(
     ON (SELECT 'temp.gz:' || line 
         FROM 
           iis_logs_raw_child_20101106
         WHERE 
           hit_timestamp >= ('20101106 0:0:0'::timestamp + interval '5 hours')
         UNION
         SELECT 'temp.gz:' || line
         FROM
...
--    ON (SELECT * from public.iis_logs_raw)
    SCRIPT('perl MRParseIISLog.1.pl') 
    OUTPUTS(
	'outfile            varchar', 
	'userid             varchar', 
	'c_ip               varchar', 
...
ALTER TABLE iis_child.iis_logs_raw_child_20101104 COMPRESS HIGH;
