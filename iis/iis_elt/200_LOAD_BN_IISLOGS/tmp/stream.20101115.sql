...
FROM STREAM(
     ON (SELECT case when srcfile is null then 'temp.gz:' else trim(srcfile) || ':' end 
         ...
         WHERE 
           hit_timestamp >= ('20101115 0:0:0'::timestamp + interval '5 hours')
         UNION ALL
         ...
         WHERE
           hit_timestamp < ('20101116 00:00:00'::timestamp + interval '5 hours')
        )
...
ALTER TABLE iis_child.iis_logs_raw_child_20101113 COMPRESS HIGH;
