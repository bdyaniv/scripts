...
/* S.SEQ_HIT_ID, */
 ...
 -- iis_mapping_cols trxm for RESOURCE_NAME
  ...
  CASE WHEN w.WORK_ID ~ '^[0-9]+$' THEN w.WORK_ID::NUMERIC
        WHEN substr(regexp_replace(WORK_ID,'^.*([\\/w|\\/p]\\/.+\\/)+',''),1,10) !~ '\\D' AND substr(regexp_replace(WORK_ID,'^.*([\\/w|\\/p]\\/.+\\/)+',''),1,10) ~ '^\\d{10}$' THEN substr(regexp_replace(WORK_ID,'^.*([\\/w|\\/p]\\/.+\\/)+',''),1,10)::NUMERIC
  ELSE NULL
  END
FROM 
  IIS_LOGS_parsed w 
...
INSERT INTO BN_HITS_SP_20111211
(
  HIT_TIMESTAMP,
  USER_ID      ,
  IP_ADDRESS   ,
...
ANALYZE BN_HITS_SP_20111211;

SELECT count(*) as BN_HITS_SP_20111211 FROM BN_HITS_SP_20111211;

TRUNCATE BN_HITS_CLIENT_20111211;
INSERT INTO BN_HITS_CLIENT_20111211
(
  HIT_TIMESTAMP,
  USER_ID      ,
  IP_ADDRESS   ,
...
ANALYZE BN_HITS_CLIENT_20111211;

SELECT count(*) as BN_HITS_CLIENT_20111211 FROM BN_HITS_CLIENT_20111211;
