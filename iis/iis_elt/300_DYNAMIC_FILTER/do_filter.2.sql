...
/* S.SEQ_HIT_ID, */
 ...
 -- iis_mapping_cols trxm for RESOURCE_NAME
  ...
  w.AREF,
  w.KEYWORD,
  CASE WHEN w.WORK_ID ~ '^[0-9]+$' THEN w.WORK_ID::NUMERIC
        WHEN substr(regexp_replace(WORK_ID,'^.*([\\/w|\\/p]\\/.+\\/)+',''),1,10) !~ '\\D' AND substr(regexp_replace(WORK_ID,'^.*([\\/w|\\/p]\\/.+\\/)+',''),1,10) ~ '^\\d{8,10}$' THEN substr(regexp_replace(WORK_ID,'^.*([\\/w|\\/p]\\/.+\\/)+',''),1,10)::NUMERIC
  ELSE NULL
  END
FROM 
  IIS_LOGS_parsed w 
...
INSERT INTO BN_HITS_SP_%CUR_YYYYMMDD%
(
  HIT_TIMESTAMP,
  USER_ID      ,
  IP_ADDRESS   ,
...
SELECT 
 ...
 -- iis_mapping_cols trxm for RESOURCE_NAME
  ...
  ELSE NULL
  END,
  w.USERID||replace(split_part(w.date_time,' ',2),':','')
FROM
 IIS_LOGS_PARSED w,
 web_click_cnt b,
 BN_SPIDER_IP p 
...
ANALYZE BN_HITS_SP_%CUR_YYYYMMDD%;

SELECT count(*) as BN_HITS_SP_%CUR_YYYYMMDD% FROM BN_HITS_SP_%CUR_YYYYMMDD%;

TRUNCATE BN_HITS_CLIENT_%CUR_YYYYMMDD%;
INSERT INTO BN_HITS_CLIENT_%CUR_YYYYMMDD%
(
  HIT_TIMESTAMP,
  USER_ID      ,
  IP_ADDRESS   ,
...
SELECT 
 ...
 -- iis_mapping_cols trxm for RESOURCE_NAME
  ...
  w.KEYWORD,
  CASE WHEN w.WORK_ID ~ '^[0-9]+$' THEN w.WORK_ID::NUMERIC
        WHEN substr(regexp_replace(WORK_ID,'^.*(\\/w\\/.+\\/)+',''),1,10) !~ '\\D' AND substr(regexp_replace(WORK_ID,'^.*(\\/w\\/.+\\/)+',''),1,10) ~ '^\\d{8,10}$' THEN substr(regexp_replace(WORK_ID,'^.*(\\/w\\/.+\\/)+',''),1,10)::NUMERIC
  ELSE NULL
  END
FROM
 IIS_LOGS_PARSED w
...
ANALYZE BN_HITS_CLIENT_%CUR_YYYYMMDD%;

SELECT count(*) as BN_HITS_CLIENT_%CUR_YYYYMMDD% FROM BN_HITS_CLIENT_%CUR_YYYYMMDD%;
