...
<<<<<<< .mine
    loader_command.write("ncluster_loader --loader 10.225.75.243 -f -D \"\" --columns uuid,line --el-discard-errors -U beehive -w b@@hive4nCluster --skip-rows 1 bna.iis_logs_raw tmp.new.txt")
=======
...
loader_command = cStringIO.StringIO()
loader_command.write("ncluster_loader --loader 10.225.75.243 -f -D \"\" --columns uuid,line --el-discard-errors -U beehive -w beehive --skip-rows 1 bna.iis_logs_raw tmp.new.txt")
p = subprocess.Popen(loader_command.getvalue(), shell=True, stdout = subprocess.PIPE)
output = p.stdout
print "output class:", output.__class__
output_data = output.read().strip()
