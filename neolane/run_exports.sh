...
arr_query[0]="select eaddress, 'DS01' as data_source, ds_stream_cycle, refresh_date, ds_stream_lifecycledate, coalesce(ereader_type,ds_stream_type) ds_stream_type, retailer_id, locale
from bn.digital_stream
...
from (
        ...
        from bn.member
                ...
                where current_term_id = term_id and (( change_pg_ts> (select max_last_pull_dt from etl.etl_export_control where feed_id=112234 and col_name_delta='change_pg_ts')
                or
                change_pr_ts > (select max_last_pull_dt from etl.etl_export_control where feed_id=1122341 and col_name_delta='change_pr_ts')
                or
                change_t_ts > (select max_last_pull_dt from etl.etl_export_control where feed_id=1122342 and col_name_delta='change_t_ts')
 ))
) z
...
for feed in "${arr_feeds[@]}"
  ...
  ##TESTING ONLY. file_backup=${base_dir}'/temp_outbound/'${feed}'-'${mydate}'.txt' # temporary backup of the exported files 5-14-2012
  ...
  let "i=i+1"
  ##TESTING ONLY. cp $file $file_backup # temporary backup of the exported files 5-14-2012
done

# ------------------------------Update Control Table Queries-----------------------------------

if [ "$OUTPUT_DIR" == "dev" ] # don't update control table if running in dev mode
then
    exit
fi
...
tail -35 $logfile | mail -s "${mydate} Neolane - exports" "ybendavid@book.com,bmarcuse@book.com,aaguiar@book.com,cdupin@book.com"
