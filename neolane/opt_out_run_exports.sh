...
# Exporting data from Aster tables bn(email_marketing,cheetahmail_demograph) to files
...
echo "Running exports" mail -s "${mydate} Neolane - exports" ybendavid@book.com

# ------------------------------Export Queries-------------------------------------------------
# DS01
arr_query[0]="select eaddress, 'DS01' as data_source, ds_stream_cycle, a.refresh_date, ds_stream_lifecycledate, ds_stream_type
from bn.digital_stream a
join bn.email_marketing b
on a.eaddress=b.email
where b.opt_out_date is null"
#where refresh_date > (select max_last_pull_dt from etl.etl_export_control where feed_id=112233)"


# PF01
arr_query[0]="select a.email, (CASE   
...
# MM01
arr_query[1]="
select z.EMAIL, z.DATA_SOURCE , MAX(MEMBER_EXPIRED_DATE) as MEMBER_EXPIRED_DATE ,
max(MEMBER_SAVINGS) as MEMBER_SAVINGS, max(MEMBER_START_DATE) as MEMBER_START_DATE, max(MEMBER_TYPE) as MEMBER_TYPE, max(MEMBERID_MASKED) as MEMBERID_MASKED, max(EDUCATOR_EXPIRED_DATE) as EDUCATOR_EXPIRED_DATE, max(EDUCATOR_START_DATE) as EDUCATOR_START_DATE, max(KC_EXPIRED_DATE) as KC_EXPIRED_DATE, max(KC_START_DATE) as KC_START_DATE, max(z.DEMO_STATE) as DEMO_STATE, max(z.DEMO_ZIP) as DEMO_ZIP, max(z.DEMO_ZIP_GROUP) as DEMO_ZIP_GROUP, max(z.DEMO_STORE_NUMBER) as DEMO_STORE_NUMBER, max(z.DEMO_STORE_NAME ) as DEMO_STORE_NAME
from (
        select
        ...
        from bn.member 
        join ( select enrollment_id , max(term_id) as m_term
                from bn.member
                where begin_dt < current_date
                group by enrollment_id ) term on term.enrollment_id = member.enrollment_id and current_term_id = m_term
                -- now lets filter by control table
--                where ( 
--                change_pg_ts> (select max_last_pull_dt from etl.etl_export_control where feed_id=112234 and col_name_delta='change_pg_ts')
--                or
...
left join bn.email_marketing b
on z.email=b.email
where b.opt_out_date is null
group by z.EMAIL, z.DATA_SOURCE
"

# PD01 - waiting for a column to filter the deltas by
arr_query[0]="
...
where (opt_out_date is null)"
# and ( inc_last_purch_dt > (select max_last_pull_dt from etl.etl_export_control where feed_id=112236 and col_name_delta='inc_last_purch_dt')
...
arr_update_feeds=() # no updates

i=0
for count in "${arr_counts[@]}"
  do
  echo "feed: "${arr_feeds[$i]}
  echo "Count: "$count
  
  if [ $count -ge 0 ]; then # update control table
      echo `date '+%Y-%m-%d %H:%M:%S'`" - Running update ${count}/${#arr_update[@]} feed: ${arr_update_feeds[$i]}" >> $logfile
      $ACT -c "${arr_update[$i]}"
  else
...
tail -20 $logfile | mail -s "${mydate} Neolane - exports" ybendavid@book.com
